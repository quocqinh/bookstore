<?php
	...
	function insertIntoOrder($conn, $customerid, $total_price, $date, $ship_name, $ship_address, $ship_city, $ship_zip_code, $ship_country){
		...
		(NULL, '" . $customerid . "', '" . $total_price . "', '" . $date . "', '" . $ship_name . "', '" . $ship_address . "', '" . $ship_city . "', '" . $ship_zip_code . "', '" . $ship_country . "')";
	...
	function getSearchBookList($search){
		$conn = db_connect();
		$query = "SELECT * FROM books b join publisher c on b.publisher_id = c.publisher_id WHERE book_title like '%$search%' OR book_isbn like '%$search%' OR LOWER book_author like '%$search%' OR book_price like '%$search%' OR c.publisher_name like '%$search%' OR SL like '%$search%'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "get book failed! " . mysqli_error($conn);
			exit;
		}
		return $result;
	}
	...
	function getSL($isbn){
		$conn = db_connect();
		$query = "SELECT SL FROM books WHERE book_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "get book amount failed! " . mysqli_error($conn);
			exit;
		}
		$row = mysqli_fetch_assoc($result);
		return $row['SL'];
	...
	function getCustomerId($name, $address, $city, $zip_code, $country){
		...
		name = '$name' AND address = '$address' AND city = '$city' AND 
	...
	function getCustomerBuy($conn){
		$query = "SELECT orderid,c.name,c.address,c.city,c.country,amount,date from orders o join customers c on o.customerid=c.customerid";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
